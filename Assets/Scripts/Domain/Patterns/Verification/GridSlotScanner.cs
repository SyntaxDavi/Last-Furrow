...
public class GridSlotScanner : MonoBehaviour
{
    ...
    [Tooltip("Pausa entre verifica��o de slots (segundos)")]
    [Range(0f, 0.5f)]
    ...
    public async UniTask ScanSequentially(RunData runData, GameEvents events)
    {
        ...
        // 1. Configurar Cancelamento (Seguran�a)
        // Se j� houver um scan rodando, cancela ele antes de come�ar o novo
        if (_cts != null) { _cts.Cancel(); _cts.Dispose(); }
        _cts = new CancellationTokenSource();
        // O token � o que passamos para os "awaits" saberem que devem parar se mandarmos
        var token = _cts.Token;

        // --- VALIDA��ES (yield break vira return) ---
        if (_gridManager == null)
        {
            Debug.LogError("[GridSlotScanner] GridManager n�o atribu�do!");
            return;
        }
        ...
        if (_uiManager == null)
        {
            Debug.LogError("[GridSlotScanner] PatternUIManager n�o atribu�do no Inspector!");
            return;
        }
        ...
        foreach (var slotView in unlockedSlots)
        {
            ...
            foreach (var pattern in patternsForSlot)
            {
                if (!_patternsAlreadyTriggered.Contains(pattern))
                {
                    ...
                    // Disparar evento de padr�o completo (para highlights)
        ...
        Debug.Log($"[GridSlotScanner] Scan conclu�do: {patternsTriggered} padr�es.");
        _patternsAlreadyTriggered.Clear();
    ...
    // ... GetUnlockedSlots (Mant�m igual pois n�o tem corrotina) ...
    ...
    {
        // Seguran�a: Garante que se o objeto for destru�do, a task para
        StopScan();
    }
}
